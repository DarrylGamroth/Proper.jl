...
run_amdgpu_benchmarks() {
  ...
  if probe_output=$(julia --project=. bench/julia/amdgpu/probe.jl 2>&1); then
    ...
    run_step "Julia AMDGPU steady-state FP32" julia --project=. bench/julia/amdgpu/steady_state_fp32.jl
    run_step "Julia AMDGPU batch throughput FP64" julia --project=. bench/julia/amdgpu/batch_throughput_fp64.jl
    run_step "Julia AMDGPU batch throughput FP32" julia --project=. bench/julia/amdgpu/batch_throughput_fp32.jl
    run_step "Julia AMDGPU core propagation tail" julia --project=. bench/julia/amdgpu/core_propagation_tail.jl
    if run_step "Julia AMDGPU supported kernels" julia --project=. bench/julia/amdgpu/supported_kernels.jl; then
      :
    else
      ...
      AMDGPU_SKIP_REASON="AMDGPU helper-kernel benchmark failed under the current compiler/toolchain" \
        julia --project=. bench/julia/amdgpu/write_skipped_supported_kernels.jl >/dev/null
    fi
  else
