...
@testset "API contract export surface" begin
    ...
    @test Base.isexported(Proper, :prepare_run)
    @test Base.isexported(Proper, :prepared_context)
    @test !Base.isexported(Proper, :PreparedRun)
    ...
    @test !isdefined(Proper, :PreparedHotCall)
    @test !isdefined(Proper, :prepare_hot_call)
    @test !isdefined(Proper, :prop_run_hot)
