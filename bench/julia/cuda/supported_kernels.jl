...
function bench_cuda_supported_kernels()
    nprop = 512
    nmap = 256
    samples = 20
    ...
    coord_xgrid = CUDA.CuArray(repeat(reshape(coord_x_axis, 1, :), nmap, 1))
    coord_ygrid = CUDA.CuArray(repeat(reshape(coord_y_axis, :, 1), 1, nmap))
...
function main()
    return write_benchmark_report(REPORT_PATH, bench_cuda_supported_kernels())
end
