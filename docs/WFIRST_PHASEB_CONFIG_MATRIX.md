...
| DMs | `use_dm1`, `use_dm2`, explicit maps | off, on with supplied maps | `compact_hlc_dm_pair`, `full_hlc_dm_pair` | Gap | Explicit HLC DM-map parity rows exist, but currently miss Python with relative L2 around `4.2e-1` and remain under investigation. |
